	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		try {
			...
			if (reqURI.indexOf(DRIVER_PATH) >= 0) {
				boolean isDriver = session != null ? (boolean) session.getAttribute(SessionBean.DRIVER_ROLE) : false;
				if (isDriver) {
				...
				boolean isPassenger = session != null ? (boolean) session.getAttribute(SessionBean.PASSENGER_ROLE)
						: false;
				if (isPassenger) {
					chain.doFilter(request, response);
				} else {
