...
public class CommunicationFacadeBean implements CommunicationFacadeRemote {
	...
	public java.util.Collection<?> showTripComments(int tripId) throws PersistenceException {
		...
		Collection<MessageJPA> questions = entman
				.createQuery("FROM MessageJPA b WHERE b.trip.id = :tripId AND b.replyQuestionId = 0")
		...
		for (Iterator<MessageJPA> iter2 = questions.iterator(); iter2.hasNext();) {
			...
			tripComments.add(q);
			try {
				MessageJPA answer = (MessageJPA) entman
						.createQuery("FROM MessageJPA b WHERE b.replyQuestionId = :questionId")
						.setParameter("questionId", q.getQuestionId()).getSingleResult();
				if (answer != null) {
